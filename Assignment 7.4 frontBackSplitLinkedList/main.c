...
{
	/* add your code here */
	int size, mid, BackSize;
	ListNode *BackList;

	if (ll->head == NULL) //if empty return
        return;
    ...
    if (size%2 == 0) //if even
    {
        BackList = findNode(ll, mid-1);
        BackSize = mid;
    }
    else //if odd
    {
        BackList = findNode(ll, mid);
        BackSize = mid+1;
    }

    resultBackList->head = BackList->next;
    BackList->next = NULL; //resets (optional)
